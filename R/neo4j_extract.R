#' Assemble a header for loading data into neo4j
#'
...
neo4j_header_relations <- function(data, relation_type_field,
  relation_src_field, relation_dst_field, ignore_fields = NULL) {
...
#
# test1 <- authors %>% select(name, kthid, orcid, orgids, extorg, pids, n_pid) %>% mutate(reltype = "AUTHOR")
#
# neo4j_header_relations(test1, relation_dst_field = "pids", relation_src_field = "name", relation_type_field = "reltype",
#                        ignore_fields = "extorg")
...
#    filter(!is.na(Contributor))
  ...
  #TODO: add also authors, issues etc and relations
  ...
  message("Done. File are in ", file.path(outdir, "import"))
}
