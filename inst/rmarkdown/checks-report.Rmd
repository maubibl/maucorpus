...
### Multipletter   
Kontroller samlade under fliken [Multipletter](https://data.bibliometrics.lib.kth.se/kthcorpus/checks-report.html#multipletter) identifierar olika förekomster av samma publikation i DiVA. Detta sker genom att hitta dubbletter, tripletter etc. med avseende på DOI, ISI, ScopusID eller titel. Om en DOI förekommer i flera poster är sannolikheten stor att så är fallet även för identifierarna ISI och ScopusID samt titel. Samma post kan alltså fångas upp i flera av nedanstående kontroller.  
...
### Formatfel 
Kontroller grupperade under fliken [Formatfel](https://data.bibliometrics.lib.kth.se/kthcorpus/checks-report.html#formatfel) identifierar ogiltigt format på diverse identifierare. Kontrollen tar inte hänsyn till om det är rätt identifierare som kopplats till publikationen utan enbart om identifieraren som angetts är giltig (d.v.s. om den har rätt format). 
...
*Ogiltigt ScopusID*     
KuraTHor fångar upp poster med ScopusID:n som: 
...
KuraTHor fångar upp poster som har ogiltiga ISSN/EISSN utifrån antal tecken samt värdet på kontrollsiffran. Ett ISSN- eller EISSN-nummer ska bestå av åtta tecken (0-9), som sista tecken är även bokstaven X tillåten och representerar då siffran tio. Det sista tecknet i strängen är en kontrollsiffra. ISSN/EISSN presenteras ofta med bindestreck mellan de fyra första och de fyra sista tecknen. Kontrollen accepterar ISSN/EISSN både med och utan bindestreck mellan det fjärde och femte tecknet. 
...
### Saknat mm 
Kontroller samlade under rubriken [Saknat mm](https://data.bibliometrics.lib.kth.se/kthcorpus/checks-report.html#saknat-mm) identifierar bland annat poster med ofullständiga uppgifter och poster med ogiltig publiceringsstatus. 
